...
async def setup_and_run(
    ...
    manager = SimulationManager(
        ...
        run_id=run_id,
        task_id=task_id,
        experiment_id=experiment_name,  # Pass experiment_name instead of experiment_id
    )
    ...
    print(
        f"🚀 Starting Schelling Simulation (Run ID: {run_id}) for {config.simulation.steps} steps..."
    )
    await manager.run()
...
def start_simulation(
    ...
    # For local runs, we need to create an experiment if one doesn't exist
    if not run_id:
        ...
        # When called from a Celery worker, the run context is already active
