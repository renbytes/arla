...
from simulations.soul_sim.components import PositionComponent, InventoryComponent, TimeBudgetComponent
...
class ExtractAction(ActionInterface):
    ...
    def generate_possible_params(self, entity_id: str, simulation_state: "SimulationState", current_tick: int) -> List[Dict[str, Any]]:
        ...
        if resource_result:
            ...
            if hasattr(pos_comp, "environment"):
                ...
                other_agents_present = any(isinstance(time_comp := simulation_state.get_component(other_id, TimeBudgetComponent), TimeBudgetComponent) and time_comp.is_active for other_id in nearby_entities if other_id != entity_id)
        ...
        if not inv_comp.farming_mode and inv_comp.current_resources >= simulation_state.config.get("farm_threshold_resources", 5.0):
    ...
    def get_feature_vector(self, entity_id: str, simulation_state: "SimulationState", params: Dict[str, Any]) -> List[float]:
        ...
        return create_standard_feature_vector(self.action_id, intent, self.get_base_cost(simulation_state), params, {"resource_id": (0, 1, 1), "establish_farm": (1, 1, 1), "is_farm": (2, 1, 1)})
