...
class BQLTestReporter:
    ...
    async def execute_and_report(self) -> Dict[str, bool]:
        ...
        if not self.assertions:
            self.console.print(
                "[yellow]BQL Info: No assertions defined for this run.[/yellow]"
            )
            return {"BQL_STATUS": True}

        self.console.rule(
            f"[bold cyan]🔬 Behavioral Unit Test Report ({self.run_id[:8]})[/bold cyan]"
        )

        test_results = Table(
            title="Assertion Details", show_header=True, header_style="bold magenta"
        )
        ...
        if tests_passed == total_tests:
            final_status = (
                f"[bold green]✅ ALL {total_tests} BEHAVIORAL TESTS PASSED[/bold green]"
            )
            panel_style = "green"
        else:
        ...
        self.console.print(
            Panel(final_status, title="Final BQL Status", border_style=panel_style)
        )
        self.console.rule()
