...
def mock_simulation_state():
    ...
    mem_comp.causal_data = [
        {"event_id": "event_123", "action": "action_a", "outcome": 1.0}
    ]
...
def test_generate_counterfactual_no_causal_model(mock_simulation_state):
    ...
    result = generate_counterfactual(
        episode, mock_simulation_state, "agent_1", "action_b"
    )
...
def test_generate_counterfactual_event_id_not_found(mock_simulation_state):
    ...
    """
    episode = Episode(
        start_tick=0, end_tick=1, theme="t", events=[{"event_id": "unfindable_event"}]
    )
