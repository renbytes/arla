...
def test_complex_always_assertion(bql):
    ...
    expected_part_1 = (
        "(components_data->'MockEnergyComponent'->>'current_energy')::FLOAT > 0"
    )
    expected_part_2 = (
        "(components_data->'MockEnergyComponent'->>'state')::TEXT = 'active'"
    )
...
def test_eventually_assertion(bql):
    ...
    assertion = bql.assert_eventually(bql.MockSocialComponent.trust_level > 0.9)
